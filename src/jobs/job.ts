...
export class Job {
  ...
  constructor({ applications, id, name, status }: JobParams) {
    this.applications = applications?.split(',').filter(Boolean).map(accountId => parseInt(accountId))
    this.id = id
    this.name = name
    this.status = status
  }
  ...
  public publish(): void | Error {
    if (this.status !== JobStatusEnum.DRAFT) {
      throw new Error(`To publish job it must be in ${JobStatusEnum.DRAFT} status`)
    }
